...
- **The `person_*` set is dominated by its per-skin-tone recolours**, generated by
  `tools/generate_skin_variants.py` from a drawn base rather than authored once per tone. That
  is why the person frames outnumber the drawings behind them.
