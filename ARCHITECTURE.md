...
| Metric | Value (measured at v4.10) | Was (v76) |
|---|---|---|
| Files / unique contents | **221 / 221** | 111 / 111 |
| Total decoded `ARGB_8888` | **27.09 MB** | 14.43 MB |
| Of which transparent padding | **4.51 MB** | — |
| Largest single sprite | `cloud_body.png` 798×396 → 1.21 MB | `cloud_body.png` 876×477 → 1.67 MB |
| Second largest | `rainbow_arc.png` 594×297 → 0.67 MB | — |
| `santa_sleigh_scene.png` | 594×123 → **0.28 MB** | 624×168 → 0.42 MB |
| **132** `person_*` sprites | 15.81 MB | 3.29 MB across 36 files |
| Off the 3× authoring grid | **0** | 0 |
| Duplicate groups | **0** | 0 |

The jump from 111 files to 221 is the three skin tones per person frame, generated by
`tools/generate_skin_variants.py`; it is why `person_*` goes from 36 files to 132. The
`cloud_body` and `santa_sleigh_scene` rows shrank because both sprites were re-cropped
after the figures above were first recorded — the old numbers were never re-measured,
which is what made this table the stalest thing in the document.
...
### Verified build (measured at v4.10)

| Task | Result |
|---|---|
| `./gradlew assembleDebug` | **BUILD SUCCESSFUL**, `app-debug.apk` 21.33 MB |
| Kotlin/Java compiler warnings | **0** |
| `./gradlew test` | **BUILD SUCCESSFUL** — 1085 tests, 0 failures, 0 errors, 0 skipped |
| `./gradlew lint` | **BUILD SUCCESSFUL** — 31 issues: 28 warnings, 3 hints, 0 errors, 0 fatal |

Lint breakdown: `UnusedResources` ×20, `UseKtx` ×3,
`AutoboxingStateCreation` ×3, plus single instances of `UnusedAttribute`,
`VectorRaster`, `ConfigurationScreenWidthHeight`, `DataExtractionRules` and
`ObsoleteSdkInt`. `OldTargetApi` is no longer among them: it fires only while
`targetSdk` lags `compileSdk`, and since v4.0 they are both 37.

These are figures from one measured run, not an invariant. The heading names the
version they were taken at so a later reader can tell a stale number from a
regression — the previous version of this table said "current environment" and
was quoting a 50-test suite.
