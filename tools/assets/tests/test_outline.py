...
OCCUPANTS = [
    ...
    # v4.19 retired the four adult vehicle bases and v4.20 the two boy ones -- in each case a
    # base that was pixel-identical to one of its own tone copies and that no draw path could
    # reach -- so those six are measured on the heir the registry's `retiredBases` names, which
    # is the same drawing under the surviving name. The girl's bases are not duplicates of any
    # of her tones and still ship, so hers are measured directly.
