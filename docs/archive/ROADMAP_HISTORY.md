...
Moved out of `ROADMAP.md` by the documentation pass of 2026-09-07. Nothing here was
rewritten: the release-by-release account, the completed list and the two entries that
had already been declared closed are reproduced unchanged.

`ROADMAP.md` keeps the current status, what is known broken, the next priorities and
what is deferred. This file keeps the record of how the project got there. What shipped
and why, in full, is `RELEASE_HISTORY.md`.

---

## The release-by-release account

This is the block that used to sit under "Current status" in `ROADMAP.md`, below the
v4.23 entry, in the same order.
...
## Closed entries removed from the live sections

Two rows had already been struck through where they stood. They are kept here for their
reasoning only; nothing in either is still open.

### From "Older priorities": item 5, orphan resources
...
### From "Deferred": D10, `targetSdk` behind `compileSdk`
...
### The note that closed "Older priorities"
