...
tag, no push, no GitHub Release. `compileSdk`/`targetSdk` remain 37. Baseline is **v4.24**, which
**is** published — read from the public GitHub API on 2026-09-08, along with v4.20 through v4.23.

**What it is.** The people are the last family drawn in the old language, and this release redraws
all of them: the walkers, the busts at the windows, and the occupants behind every windscreen.
Concept **B "Rilievo" with the suggestion of a face** was chosen by the maintainer from
photographs taken at real size on the device. Alongside it, the GL path gained a stable reduction
for sprites drawn much smaller than they are authored, which is what the small figures needed to
stop shimmering as the scene scrolls.

### What the user gets

`release-notes/v4.25.md`: the people redrawn with faces, the occupants of every car facing the way
the car is going instead of out of the back window, children drawn as children, heads in the
proportion the rest of the scene is drawn in, and the small figures no longer crawling along their
edges while the scene scrolls. No setting was added, removed or renumbered; no saved theme is
touched.

### The artwork

- **40 SVG sources committed**, the shipped PNGs are the pipeline's own output, and the 132
  per-skin-tone recolours were regenerated by `tools/generate_skin_variants.py`. `validate` reports
  266 entries, 140 with an SVG source, 126 recorded gaps.
- **Every canvas was trimmed onto what the family actually covers** — 41×85 → **39×84**, 53×57 →
  **49×57**, 47×44 → **38×42** units — with the anchors compensated in the same change, which is
  what makes a crop safe. **1 630 656 B of decoded budget freed — 1.63 MB, 1.56 MiB** — measured
  by decoding both shipped sets: 30 254 580 B at v4.24 against **28 623 924 B** here, leaving
  **1 784 780 B** under `SpriteGeometryTest`'s 29 MiB ceiling. (The artwork pass recorded 1.67 MB
  and then 1.82; the figure above is what the shipped sets measure now that the proportion pass
  has given the two bust canvases a unit of width back. `tools/assets/reports/runtime-inventory.md`
  was regenerated for it.)
- **The eyes were a single refinement, one attempt.** At `2.7r` the gap between them measured
  0.53 px of screen and read as a bar; at `4r` it measures 1.52 px, the width of an eye, and the
  three marks read as three.

### One rule for a head, and the four defects it uncovered

`PERSON_HEAD_SPRITE_UNITS` and `HEAD_CAR_HEAD_UNITS` had been measured by two different rules —
the walker's head as "the end of the first run of skin rows", the bust's as "crown to chin". B
"Rilievo" draws the neck in skin, so the first rule started reading a head **half again too tall**
and the occupants came out **17% too small** on the device. The rule is now one: *the jaw is where
the skin narrows into the neck*, applied to both poses and to every site that measures one.

Tightening the generator against that rule surfaced four more:

1. the declared crown was not the drawn one (0.00 / 2.67 / 0.00 / 2.67 against a declared 2.0); the
   generator now measures the headwear's overhang and lowers the head by it. Crown-to-chin spread
   across the adults went from **4.33 to 0.67 units**;
2. a child was **98%** of an adult where the size table has always said 90%. Now **0.890**;
3. the seated head band overflowed the seat pitch on **seven busts of eight** — declared 22 units,
   drawn to 26. Now **22.33** against a pitch of 23, counting the under-paper and the cut wobble;
4. the winter child's bust reserved six units for a hat it does not wear.

The three constants loosened while that was being chased — the child factor, the crown-to-chin
tolerance and `OCCUPANT_SEATED_FIT` — are back at their declared values.

### The seated occupants: a squashed head, a reversed face, and a seat pitch

Three defects were present in the concept from phase 1, survived approval, and were found one at a
time after it. All three are corrected at the cause.

- **The heads were narrow because a constraint was read in the wrong unit.** The generator's
  `SEATED_HALF_BAND` — the band that keeps two seated occupants clear of each other — was written
  in the bust's units and justified against a seat pitch in the car's. A bust unit is **0.5255**
  car units, so the band was twice as tight as the car is, and the head was drawn down to fit it:
  `head_rx` **9.0** against **18.0** for the same person at a window, and the seated family
  shipping at **0.61–0.72** of width over height where every other drawing of those people sits
  between 0.96 and 1.52. Nothing failed, because every size assertion in `OccupantHeadFitTest` was
  about a head's *height*, which was right. The band is derived through the conversion now, the
  oval is back at the family's own proportion — **1.01 / 1.03 / 1.00 / 1.16** in a car — and the
  hair follows the head, every horizontal extension a multiple of the head's radius instead of a
  fixed offset tuned on the narrow drawing. A new assertion, *a head is as round wherever the same
  person is drawn*, holds every head to 0.85 of its own height, with the mutation to prove it: the
  shipped-narrow bust reads 0.64 and fails.
- **Every occupant in every car was facing the rear.** Not a missing mirror — `drawCar` already
  turns the whole vehicle, busts included, with the direction of travel — but a constant sense:
  the three-quarter artwork carries its hair mass toward −x and therefore its face toward +x,
  which is the vehicle's back. It was invisible until this release because the family it replaced
  was frontal and symmetric. `drawSeatedOccupant` mirrors on x, about the seat anchor rather than
  the canvas centre so the eye axis does not move. `BACKLOG_v4_25.md` item 62.
- **The seat pitch went 23 → 21.5.** With the head at its proper width, the saloon's pillar
  criterion — 15% of a head's width of glass between a head and its pillar — was short by 1.1 car
  units on every saloon at every seat offset, and moving the pair only chose which pillar the
  shortfall landed on: the plain and police saloons cross at 12–13% and never both reach 15%. The
  pitch is what gives it back, and it gives it back to a spacing the app has already shipped: at
  21.5 the two occupants' ink comes no closer than **2.06 car units**, against **1.27** on v4.24's
  family at the old 23. `CarShell.seatOffsetXUnits` (0 / −1.4 / 0) stays, because the police
  saloon's livery bands the low glass and shortens the pane that is left.

Measured after all three, on all six cabins: compact **28.0%**, saloon **18.0%**, estate **30.0%**,
taxi **30.0%**, police saloon **18.0%**, fire engine **30.6%**, against a 15% criterion — and that
criterion now measures every cabin before asserting instead of stopping at the first, which is how
the compact's 24% had been covering the police saloon's 10% for a whole round.

### The stable reduction

`SpriteDetailLevel`, `GlTextureCache` and `GlSceneTarget` keep a copy of a sprite at about twice
the size it is drawn at, so a figure blitted at a fifth of its authored size no longer samples one
texel in five. Measured on the BV6600, release-like build, Autumn, fixed clock, 180 s warm-up,
60 s windows, n=3, the two arms differing by one line:

| | levels off | levels on |
|---|---:|---:|
| process, visible | 50.81% of one core (sd 0.22) | **47.58%** (sd 0.48) |
| fps from the SurfaceFlinger layer | 29.66 | **29.66** |
| process, hidden | 0.116% | **0.138%** |
| CPU ms per frame | 17.13 of 33.7 | **16.04 of 33.7** |

Shimmer **−52%** and softening **−30%** across eleven sprites at their measured sizes. Texture
memory does not fall (27.7 against 28.9 MB, inside a 27.3–30.7 MB noise band): the atlas' fixed
16 MiB slab absorbs the texel saving.

### The goldens

Regenerated twice in this release — once for the artwork, and again here after the heads, the
facing and the seat pitch moved the occupants. The second pass is the one the tree ships, and it
followed the v4.23 protocol without any part of it being relaxed:

- **Attribution before regeneration.** The 25 committed Canvas frames were compared against freshly
  rendered ones by the golden's own rule (8 levels per channel): **2 877 pixels changed across all
  25 frames, 0 of them outside the three bands the people occupy** — facades 376–636, pavement
  546–655, road 626–703 — with the extreme rows touched at **508 and 675**. Broken into connected
  components the change is 16 to 51 small clusters per frame, the largest **64 px**, and every one
  of them sits on a person: a bust behind a shop or tower window, or the two occupants behind a
  windscreen. No bodywork, no building, no tree, no sky.
- **The four gate rectangles measure 0.0000%**, verified the way the protocol asks: two
  independent instrumentation runs, **25 of 25 frames byte-identical** between them, and inside
  each gate rectangle 0 differing pixels of 27 720 (`CAR_COUNT_GATE`), 27 720 (`CAR_NIGHT_GATE`),
  93 600 (`BUSINESS_HOURS_GATE`) and 39 240 (`PEOPLE_DENSITY_GATE`).
- **No tolerance moved**, and it is a diff rather than a claim: `SceneGolden.kt`, `SettingsGates.kt`
  and `GoldenScene.kt` are byte-identical to the v4.24 archive, and `GlGolden.kt` differs only by
  an added log line.
- **The three GL references were not touched**, and their scenes' Canvas frames did change, which
  is the deliberate consequence recorded as item 56.

Counted as assertions rather than as files, which this project has got wrong before: **26 Canvas
assertions over 25 PNGs, 3 GL assertions over 3 PNGs, 28 PNGs in the directory.**

### What the goldens found, and neither was the scene

- **A vehicle detector living on a cliff edge.** `CarNightCrossfadeTest` reported a car vanishing
  mid-road at dusk. Nothing vanishes: across frames 124–131 the far-lane car is in the middle of
  the road in every one, and the *detector* loses it for three. As dusk darkens the body, its dense
  column run falls from 26 columns to 9, under a minimum of 12. `VEHICLE_COLUMN_DEPTH_SHARE` was
  re-derived by the method the file itself declares — weakest column of the car to accept **0.167**
  (tenth percentile), lane strip to reject **0.062** (median), constant halfway — from **0.20 to
  0.12**. There is nothing in between: at 0.16 and below the run is 47–54 columns in every frame,
  at 0.20 it is 9–26. The constant had been on that cliff since it was last re-derived for the same
  reason, when the saloon was redrawn.
- **A test normalising a bust by a pedestrian's fringe.** `VehicleOccupantScaleTest` divided *both*
  sides of its occupant-versus-pedestrian ratio by the walking family's visible skin, bust included.
  The old artwork hid it by leaving similar amounts of face uncovered in both poses.

Both defects predate this release and neither had been seen.

### The tests, and a quantity that is not a dimension

Three instrumented assertions were left failing by the artwork pass, deliberately and without being
loosened. All three are resolved here, and none of them by widening a band:

- **Two were measuring the visible skin as though it were a size.** It is a property of the
  haircut: across the four adult drawings the skin's share of the head block runs **0.861 / 0.794 /
  0.623 / 0.733**, so two poses of one person cannot agree on it however correctly they are scaled.
  They measure the **head block** now — crown of the hair to the jaw, the thing the size table
  actually declares — read off the painted frame by the same rule `OccupantHeadFitTest` applies to
  the PNG. Across all twelve vehicle-and-lane cases the drawn head lands between 1.3 px under and
  2.7 px over the prediction, with the sign following the vehicle rather than the lane: two
  rasterised edges on a 43–53 px measurement. The mean is held to 1.5 px — tighter, in relative
  terms, than the 1.5 px the test used on a 31 px face — and each case to 4 px.
- **The pedestrian half of the comparison was retired rather than rebuilt**, because a pixel cannot
  support it: a walker's ground row is not in the frame (the scene's soft shadows put an ink walk
  thirty pixels below the heel), the two crowns are found against different backgrounds, and at
  full density the two nearest adults are standing in front of somebody else. The criterion is
  covered exactly where it can be: `VehiclePedestrianScaleTest` asserts it on the arithmetic and
  `OccupantHeadFitTest` on the shipped PNGs. Mutating `PERSON_HEAD_SPRITE_UNITS` by the 17% this
  release's first build was wrong by fails **six** assertions across those two classes — and passed
  the rebuilt pixel version.
- **The third was not about skin at all, and the first answer to it was wrong.** rc5's criterion —
  the occupants fill at least half the glass — was failing, and the release's first build lowered
  its floors to 40% and 35% on the reading that a three-quarter drawing simply fills less pane. It
  does not. The heads were failing that criterion because they had been **drawn narrow to satisfy
  a constraint written in the wrong unit**, and the criterion was doing exactly its job. The
  floors are **0.50 and 0.50** again, unchanged from every release before this one, and the
  artwork passes them. The sentence used to justify the move — that the v4.24 heads were as wide
  as the seat pitch, touched, and read as a wall of faces — was not a measurement, and does not
  survive one: measured over every seatable pair and every row, v4.24 left **1.27 car units** of
  clear glass between the two occupants and v4.25 leaves **2.06**. `BACKLOG_v4_25.md` item 59
  carries the correction.

### Two gates that were reading a copy of the artwork

- **`SpriteTintClassTest` could not see a tint applied at the blit.** It reads the shipped PNGs and
  its two lists of names are the call sites restated by hand, so a call site that starts
  multiplying finished art by a colour leaves every one of its assertions green.
  **`SpriteTintAtBlitTest`** closes it from the other side: the scene is driven through a recording
  ...
  the white identity, a colourless mask must never be. No list of names is involved. Proved by
  mutation: darkening `SpriteBlitter.draw` by 1% leaves all five `SpriteTintClassTest` assertions
  green and fails the new one.
- **The occupant colour table had two dead entries.** `VehicleOccupantScaleTest` scans for the
  people's non-skin colours to check that nothing of an occupant is painted outside its pane, and
  the palette still held the previous family's: the woman's hair as `F7CE64` (now her hairband and
  the girl's shirt) and the boy's shirt as `6BA84F`. Neither is a missing pixel — it is a pixel the
  scan cannot see, which makes a gate quietly weaker rather than noisily wrong. Re-measured against
  the shipped PNGs (`8C5A38` covers 52 592 pixels, `3F8A4A` 61 029), and a new assertion fails if
  any scanned colour stops appearing in the artwork.

### Where the second render thread comes from

Two kernel threads named `PaperScrapeGlTh` had been measured in the wallpaper process in every
condition, with a leaked settings preview as the standing hypothesis. **It is neither a leak nor
ours.** The process holds exactly one Java thread of that name while `/proc` shows two; profiled
with `simpleperf`, the second is 75% kernel, 11% `libsrv_um.so` (PowerVR's user-mode driver), 3%
`gralloc`, and contains no `libart` frame at all. It wears our name because Linux gives a new
thread its creator's `comm` and the driver never renames it — in a process where the settings UI
touches EGL first, the same thread appears as `RenderThread`. It costs 4.45% of a core while the
wallpaper draws and does not exist while it is hidden. `CLAUDE.md` and `ARCHITECTURE.md` carried a
per-thread CPU figure as though the render thread were one thread; both now say to quote the
process figure. `BACKLOG_v4_25.md` item 60.

### What was deliberately not done

- **The three GL reference frames were not re-authored**, so they still portray the previous
  drawing of the people. They are tied to the OnePlus 6T's Adreno driver and re-capturing here
  would move the reference onto PowerVR, which is a release of its own. They pass because the
  metric they carry — contour displacement, 0.05 / 0.05 / 0.24% against a 3% gate — is insensitive
  to figures that occupy a few hundred pixels of a frame. `BACKLOG_v4_25.md` item 56.
- **Twenty window-occupant sprites that no draw path can reach were kept.** The twelve winter
  recolours are unreachable *by design* — `seasonIndexFor(Exposure.INDOORS)` is 0 whatever the
  theme does, because nobody stands at their own window in a coat — and the eight bases are
  unreachable because the table naming them is read by nothing. 98.3 KiB of a 21.88 MiB APK, and
  nothing at runtime, since they are never decoded. `BACKLOG_v4_25.md` items 57 and 58.
- **Paper grain (`BACKLOG_v4_23.md` item 42) was closed as rejected**, on the measurement rather
  than on taste: the darkest tintable sprite is `bar_cornice` at a mean of 225.33, so the colour
  gate's floor of 220 leaves the whole set 2.36% of mean darkening to spend — a twentieth of the
  overlay v58 deleted for rendering a light-blue sky grey.

### Documentation

`DESIGN_NOTES.md`'s size table was checked against the code and **four rows were wrong**: the
pumpkin read 0.5 m against 1.0, the car 1.45 m / 48 units against 1.6912 / 56, the tower
16.8 / 196 against 15.6 / 182, and a hot-air balloon had a row although `SceneObjectType` has had
no balloon in it for releases. The reference scale read **40 local units per metre** where the code
has said **45** since v2.5, and the lake's own metric read 15 px per metre against 21. The table
now carries the two commands that recompute it.

`DESIGN_NOTES.md` gains three sections this release. Two record what was decided — *nothing narrows
a head to make it fit something*, and *an occupant faces the front, and a symmetric drawing hides
which way that is*. The third is **the acceptance sheet** (§14), and it is a condition on the
approval process rather than a note: five things a judging image has to carry — every sprite at
its on-screen size inside a real frame, every facing shown both ways, every sprite inside the
geometry that constrains it, the current artwork beside it at the same scale, and the same subject
at the same instant on both sides of a comparison — plus one composition rule, *no buried
sub-family*. Each is written with the episode that earned it. `AI_PROJECT_RULES.md` 13.6 points at
it and says what it means: an image without those cannot show the defects they exist to expose, so
approving it approves nothing. **This release cost four correction rounds after approval, for three
defects that were all present and visible in phase 1 and that no judging image displayed.**

Three documentation defects were found in passing and corrected; they are one family
(`BACKLOG_v4_25.md` item 63). **The redraw left six stale sizes standing in comments** — the walk
canvas quoted as 123×255 px when it is 117×252, the window bust as 53×57 units when it is 49×57,
the seated bust as 47×44 when it is 38×42, and the seated artwork described as "frontal" when it is
three-quarter. `SpriteMeasurementClaimTest` exists to catch exactly that and caught none of them,
because it matches one written shape (`` `sprite_name` … is NxM px ``) and not one of the six was
written that way. **`drawWindowOccupant` cited `WindowOccupantScaleTest` as the thing that pins its
two halves; that class does not exist, and does not exist in the v4.24 archive either** — the test
that actually does it is `OneOccupantRuleTest`. And a KDoc block describing `drawCar` had been
stranded above `drawSeatedOccupant`, leaving one function with two doc comments and the other with
none.

`tools/assets/reports/runtime-inventory.md` and its JSON were **regenerated**: they still described
the v4.24 artwork, sprite by sprite, after 266 PNGs had been replaced.

### Verification

Level 3. `testDebugUnitTest` **1 346 tests, 0 failures, 0 errors**; `lintDebug` **0 errors**
(27 warnings); `assembleDebug` successful, 22 977 868 B; asset tooling `probe` matching,
`validate` OK, **108 tooling tests OK**; the instrumented suite **150 tests, 0 failures,
0 ignored** in 2 450 s on the BV6600 — one more test than v4.24's 149, which is
`SpriteTintAtBlitTest`; `VehicleOccupantScaleTest` keeps its count, having lost the pedestrian
guard and gained the palette guard. A clean extraction of the delivery ZIP rebuilds with
`--no-build-cache`, 53 of 53 tasks executed, 19 Kotlin compiler warnings and the same 1 346 tests.

**Four mutations, and the fourth is a finding.** Removing `drawSeatedOccupant`'s mirror — putting
every occupant back to facing the vehicle's rear — fails **one** assertion of 45, and it is the
pillar-clearance one rather than anything about facing: all 26 Canvas golden assertions, all three
GL ones, `VehicleDrawOrderTest` and `theOccupantsFillHalfTheGlass` stay green. Nothing in the
project asserts which way a person is looking; what caught it was the saloon having no margin to
spare. Recorded in `BACKLOG_v4_25.md` item 62.
