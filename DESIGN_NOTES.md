...
| 27 | **A person's colour is resolved at the blit, from four regions carried as weight masks.** A shape ships as fixed art plus up to four masks — skin, head, shirt, trousers — and the engine draws `fixed + Σ (mask × colour)`. The masks are **summed**, never laid over. A region belongs to a **piece** the generator drew, never to a colour it happens to be painted in. | v4.30 | Shipping a copy per colour multiplies the whole person set by every axis added: hair, shirt and trousers the old way would have been 27 copies of every figure, which is a road that ends. Summing rather than overlaying is not a matter of taste — two source-over layers split the pixel's coverage, `a + b(1-a)` is not linear, and once each is halved separately for upload the figure grows a halo measured at 63 levels of coverage out of 255. And deciding a region by colour does not work: the man's hair, his shoes, his eyes and his ground shadow are all `#2B2A33`, and 868 of the 3 167 pixels of that colour on one frame are not hair. | Every person: walkers, the carrying pose, window busts, car busts |
| 28 | **The head region is what is on the head, and headwear wins.** A bare head has its hair recoloured from black / brown / blond; a covered one has the cap or hat recoloured from red / blue / green. **Nobody's headwear is removed to make room for hair.** The winter boy has no head region at all, because what he wears is his own coat's hood. | v4.30 | Thirteen of the fifty-two live shapes show no hair whatever — the boy wears a cap in summer and a hood in winter, the girl a hat in winter — so an axis that only applies to hair would apply to none of them. Taking the cap off is a pass of artwork and an aesthetic decision, not a choice of format. And the hood is drawn in the coat's own paint and is continuous with it: colouring it apart would cut a seam the drawing does not have, so it changes colour with the coat instead. | Every person |
| 29 | **Shirts and trousers are dealt as complete outfits, not as two independent colours.** Five pairs, every colour one the wallpaper already prints. | v4.30 | Two independent lists put a yellow shirt over cream trousers as readily as over charcoal, and a street dealt that way reads as a costume box rather than as people. It is the same reasoning that made the umbrella canopies a list of five chosen colours instead of a hue range. A summer woman and a summer girl have no lower garment at all — their legs are painted in their own skin — so for them the outfit is the top and the trouser region is empty, which is correct and not a hole. | Pedestrians, window busts, car busts |
| 30 | **A walker is dealt fresh colours on each crossing, and only while no copy of it is on screen.** Deterministic: at a given instant a given figure is a given colour, on every device and every run. Driven by the **clock**, not by uptime. | v4.30 | Colour used to be a pure function of the theme, so a street was dealt once and held that hand forever — on *Big City*, three fair-skinned men and three dark-skinned children for as long as the theme was on. Real randomness is refused: it looks the same, because in both cases the colour only moves off screen, and it would destroy `people-skin`, the only automatic check the colour system has. Counting off uptime is refused too — it restarts with the process and would deal the same opening hand every time. **The crossing counter alone is not an off-screen guarantee**: a pedestrian is tiled, so the wrap hands the figure from one copy to the next without moving it, and that instant is in view 52.8 % of the time. The gate is the cull that actually ran. | Pedestrians, and the umbrella one carries |
| 34 | **A child is 0.65 of an adult, and is not an adult drawn small.** The head keeps the size it had; the height comes off the legs first and then the torso, and the stride and arm swing shorten with the legs. | v4.30, chosen by the maintainer from photographs | At 0.779 the figure had a ten-year-old's proportion and, worse, an adult's build: the same 31 % of leg. Of three proportions photographed at 1× on the device, 70 % still read as a short adult beside a grown-up and 60 % was a dot at night in a crowded scene; 65 % is the first at which the child reads as a child and still has a figure. A five-year-old's head is 85–90 % of an adult's and the body far less, which is why the head does not shrink — and it is also what keeps the walker's head matching the busts in cars and at windows, which are untouched. | The four walk families |
...
| D3 | Do people become a fully customisable category (visibility + density + colours) or remain ambient with a single toggle? | **Open, and v4.30 changed what it would cost.** The colours are resolved at the blit now, from `PeopleColours`, so "let the user choose the shirts" went from redrawing the set per value to exposing a palette. **Skin is not part of that question and must not become part of it** — it is an automatic property of a generated person and `SkinToneAssetsTest` enforces that against the sources, which is why this row says *colours* and the artwork says *tone*. | People integration |
